...
let command bootloader q pv_bootloader_args image vm_uuid = 
  ...
  let pv_bootloader_args = if pv_bootloader_args = "" then [] else Stringext.String.split ' ' pv_bootloader_args in
...
let parse_exception x =
	...
	match Stringext.String.find_all runtimeError x with
	| i::_ ->
