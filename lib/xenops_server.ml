  ...
  let push t tag item =
    Debug.with_thread_associated "queue"
      (fun () ->
         Mutex.execute m
           (fun () ->
              let real_tag, aliased = if StringMap.mem tag !aliases then StringMap.find tag !aliases, true else tag,false in
              let q, redirected = if StringMap.mem real_tag !overrides then StringMap.find real_tag !overrides, true else t.queues, false in
...
and perform_exn ?subtask ?result (op: operation) (t: Xenops_task.task_handle) : unit =
  ...
  let one = function
    ...
    | VM_migrate (id, vdi_map, vif_map, vgpu_pci_map, url') ->
      ...
      let regexp = Re_pcre.regexp id in
      ...
      Redirector.alias Redirector.default ~tag:id ~alias:new_src_id;
    ...
    | VM_receive_memory (id, final_id, memory_limit, s) ->
      if (final_id <> id) then
        Redirector.alias Redirector.default ~tag:id ~alias:final_id;
...
module VM = struct
  ...
  let receive_vgpu uri cookies s context : unit =
    ...
    Debug.with_thread_associated dbg
      (fun () ->
         let vgpu_id = Debug.with_thread_associated dbg
