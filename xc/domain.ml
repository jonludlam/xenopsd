 ...
 *)
let restore_libxc_record (task: Xenops_task.t) ~hvm ~store_port ~store_domid ~console_port ~console_domid ~no_incr_generationid ~extras xenguest_path domid uuid fd =
...
let restore_common (task: Xenops_task.t) ~xc ~xs ~hvm ~store_port ~store_domid ~console_port ~console_domid ~no_incr_generationid ~vcpus ~extras xenguest_path domid fd =
	...
	| x when x = old_save_signature ->
		...
		let fd_uuid = Uuid.to_string (Uuid.create `V4) 
		and pipe_w_uuid = Uuid.to_string (Uuid.create `V4) in
		...
		and (xenguest_th, xenguest_st) =
			spawn_thread_and_close_fd "xenguest" pipe_r (fun () ->
				restore_libxc_record task ~hvm ~store_port ~store_domid ~console_port ~console_domid ~no_incr_generationid
	...
	| x when x = new_save_signature ->
		...
		let rec process_header res =
			...
			| Libxc, _ ->
				...
				let res = restore_libxc_record task ~hvm ~store_port ~store_domid ~console_port ~console_domid ~no_incr_generationid
